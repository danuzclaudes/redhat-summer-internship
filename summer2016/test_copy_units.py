...
def _get_rpm_dependencies(server_config, repo_id, rpm_name):
    ...
    res = []
    stack = []
    stack.append(rpm_name)
    while len(stack) > 0:
        top = stack.pop()
        if top is not rpm_name: res.append(top)
        ...
        if len(parent_modules) == 0: continue
...
class CopyAndPublishTwoVersionsRepoTestCase(CopyBaseTestCase):
    ...
    This test targets `Pulp Smash #311`_. The test steps are as follow:
    ...
    def test_01_copy_older_publish(self):
        ...
        cli.Client(self.cfg).run(
            'sudo yum-config-manager --add-repo {0}{1}'
            .format('https://dev/pulp/repos/', self.repo_id).split()
        )
        cli.Client(self.cfg).run('sudo yum repolist enabled'.split())
        # Execute `yum install` to deploy the RPM on the host.
        completed_proc = cli.Client(self.cfg).run(
           'sudo yum install -y --nogpgcheck {}'.format(self.rpm_name).split()
        )
        self.assertIn('Installed:', completed_proc.stdout.splitlines())
    ...
    def tearDownClass(cls):
        ...
        cli.Client(cls.cfg).run(
            'sudo yum remove -y {}'.format(cls.rpm_name).split()
        )
        # Delete the published repositories.
        cli.Client(cls.cfg).run(
